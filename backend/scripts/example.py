...
def example_multi_turn():
    ...
    result = app.invoke(
        {"messages": [HumanMessage(content="I'm planning a bike trip from Edinburgh to London")]},
        config
    )
    ...
    result = app.invoke(
        {"messages": [HumanMessage(content="I want to do about 100km per day")]},
        config
    )
    print(f"Assistant: {result['messages'][-1].content}\n")
...
def example_error_handling():
    ...
    try:
        ...
        result = app.invoke(
            {"messages": [HumanMessage(content="Rome to Florence, 70km/day")]},
            config
        )
        print("✓ Success\n")
    ...
    try:
        ...
        result = app.invoke(
            {"messages": [HumanMessage(content="XYZ123 to ABC456, 100km/day")]},
            config
        )
...
def example_custom_configuration():
    ...
    result = custom_app.invoke(
        {"messages": [HumanMessage(content="Prague to Vienna, 90km daily")]},
        config
    )

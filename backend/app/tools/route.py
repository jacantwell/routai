...
def get_route(origin: Coordinate, destination: Coordinate) -> Route:
    ...
    try:
        ...
        routes_response = response.json()["routes"][0]

        route = Route(
            polyline=routes_response["polyline"]["encodedPolyline"],
            distance=routes_response["distanceMeters"],
            duration=routes_response["duration"],
        )
...
def generate_waypoints(
    route_polyline: str, daily_distance_km: int, total_distance_km: float
) -> list[Waypoint]:
    """
    ...
    num_days = int(np.ceil(total_distance_km / daily_distance_km))
    num_waypoints = num_days - 1  # Last day ends at destination

    waypoints = []
    cumulative_distance = 0.0
    target_distance = daily_distance_km
    day_number = 1
    ...
    for i in range(len(coordinates) - 1):
        ...
        if cumulative_distance >= target_distance and len(waypoints) < num_waypoints:
            ...
            waypoint = Waypoint(
                day=day_number,
                coordinate=Coordinate(
                    latitude=Latitude(point2[0]), longitude=Longitude(point2[1])
                ),
